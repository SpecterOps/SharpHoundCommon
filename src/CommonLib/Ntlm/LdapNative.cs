...
public class SafeLdapHandle : SafeHandleZeroOrMinusOneIsInvalid {
    ...
    protected override bool ReleaseHandle() {
        if (!IsInvalid) {
            var result = NativeMethods.ldap_unbind_s(handle);
            return result == 0;
        }
...
public class LdapConnection : IDisposable {
    ...
    private string? GetServerError() {
        ...
        try {
            var result = NativeMethods.ldap_get_option(
                Handle,
    ...
    public void CheckError(int result, string operation) {
        if (result == (int)LdapErrorCodes.Success) return;

        var lastError = NativeMethods.LdapGetLastError();
        var serverError = GetServerError();
        throw new LdapNativeException(
    ...
    public void Initialize(string hostName, int port) {
        ...
        if (handle == IntPtr.Zero) {
            var lastError = NativeMethods.LdapGetLastError();
            throw new LdapNativeException($"Failed to initialize LDAP connection", lastError);
        }
    ...
    public void Connect(int timeout = -1) {
        ...
        try {
            ...
            var result = NativeMethods.ldap_connect(Handle, timeoutPtr);
    ...
    private T GetOption<T>(LdapOption option) where T : struct {
        ...
        if (typeof(T) == typeof(int)) {
            int value;
            var result = NativeMethods.ldap_get_option(Handle, option, out value);
    ...
    public void SetOption(LdapOption option, LdapOptionValue value) {
        ...
        // Get current value
        var currentValue = GetOption<int>(option);
        var newValue = (int)value;

        // Only set if different
        if (currentValue != newValue) {
#pragma warning disable CS8604 // Possible null reference argument.
            var result = NativeMethods.ldap_set_option(_handle, option, ref newValue);
#pragma warning restore CS8604 // Possible null reference argument.
            CheckError(result, $"Setting LDAP option {option}");

            // Verify the change
            var verifyValue = GetOption<int>(option);
            if (verifyValue != newValue) {
                throw new LdapNativeException(
                    $"Failed to verify option change for {option}. Expected {newValue} but got {verifyValue}",
...
#pragma warning disable CS8604 // Possible null reference argument.
        var result = NativeMethods.ldap_set_option(_handle, option, value);
#pragma warning restore CS8604 // Possible null reference argument.
        CheckError(result, $"Setting LDAP option {option}");
    }
    ...
    public byte[] SaslBind(string distinguishedName, string mechanism, byte[] credential) {
        ...
        try {
            ...
            // Perform SASL bind
            var result = NativeMethods.ldap_sasl_bind_s(
                Handle,
                distinguishedName,
                mechanism,
                credPtr,
            ...
            if (result != (int)LdapErrorCodes.Success &&
                ...
                throw new LdapNativeException("SASL bind failed", result, GetServerError());
            }

            var bindResultCode = GetResultCode();

            if (bindResultCode != (int)LdapErrorCodes.Success &&
                bindResultCode != (int)LdapErrorCodes.SaslBindInProgress) {
                throw new LdapNativeException("SASL bind result failed", bindResultCode, GetServerError());
            }
            ...
            var responseData = new byte[responseBerval.bv_len];
