...
";
        ...
        private List<NtlmSession> CollectEvents(string query) {
            ...
            using var logReader = GetEventLogReader(host, "Security", query);
            ...
            for (var evnt = logReader.ReadEvent(_readEventTimeoutMs);
                ...
                if (readEventDelayMs > 0) {
                    System.Threading.Thread.Sleep(readEventDelayMs);
                }
            }
        ...
        private async Task<string?> ResolveNameToSid(string username, string domain) {
            var (matchSuccess, sids) = await utils.GetGlobalCatalogMatches(username, domain);
            if (matchSuccess) {
                if (sids.Length > 1) {
                    log.LogError("More than one SID was returned for the user '{user}'. SIDs: {sids}", username,
                        string.Join(",", sids));
                    return null;
                }
                ...
                var (success, principal) = await utils.ResolveAccountName(username, domain);
        ...
        private async Task<NtlmSession> ProcessSecurityEvent(EventRecord evnt, SecurityLogId eventId) {
            ...
            switch (eventId) {
                case SecurityLogId.Logon:
                    session = NtlmSession.FromLogonEvent(evnt);
                    return session;

                case SecurityLogId.ValidateCredential:

                    session = NtlmSession.FromValidateCredentialEvent(evnt);

                    // The Validate Credential event does not return the domain or account sid
                    // so try and find those
                    var sid = await ResolveNameToSid(session.AccountName, domain);

                    session.AccountSid = sid?.ToString();
                    session.AccountDomain = domain;
        ...
        private EventLogReader GetEventLogReader(string host, string path, string query) {
            ...
            if (!string.IsNullOrEmpty(host)) {
                ...
                if (localAdminCredential == null) {
                    ...
                    session = new EventLogSession(
                        host,
                        localAdminCredential.Domain,
                        localAdminCredential.UserName,
                        localAdminCredential.SecurePassword,
                        SessionAuthentication.Negotiate
                    );
                }
