...
namespace SharpHoundCommonLib.SMB {
    ...
    public class SmbScanner : ISmbScanner {
        ...
        public SmbScanner(ILogger log) {
            ...
            _adaptiveTimeout = new AdaptiveTimeout(defaultTimeout: TimeSpan.FromMilliseconds(TimeoutMs), Logging.LogProvider.CreateLogger(nameof(TrySMBNegotiate)), sampleCount: 100, logFrequency: 1000, minSamplesForAdaptiveTimeout: 30);
        }
        ...
        private async Task<SharpHoundRPC.Result<SmbScanInfo>> TrySMBNegotiate(string host, int port, bool useSMB1 = false) {
            try {
                ...
                var scanInfo = new SmbScanInfo(host) {
                    SigningRequired = CheckResponseRequiresSigning(negoResp.Value, useSMB1)
                };
        ...
        /// </summary>
        private async Task<byte[]> SendAndReceiveData(string host, int port, byte[] data, CancellationToken cancellationToken) {
            ...
            int bytesRead = await stream.ReadAsync(responseBuffer, 0, responseBuffer.Length, cancellationToken);
