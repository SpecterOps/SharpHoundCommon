...
namespace CommonLibTest {
    public class GPOLocalGroupProcessorTest {
        ...
        *S-1-5-32-544__Members = 
        ...
        private readonly string GpttmplInfContentWithBackup = @"[Unicode]
        Unicode=yes
        [Version]
        signature=""$CHICAGO$""
        Revision=1
        ...
        *S-1-5-21-3130019616-2776909439-2417379446-514__Members =
        *S-1-5-32-544__Members =
        *S-1-5-32-551__Members = *S-1-5-21-3130019616-2776909439-2417379446-513
        ";

        private readonly string GroupXmlContentWithBackup = @"<?xml version=""1.0"" encoding=""UTF-8""?>
        <Groups clsid=""{3125E937-EB16-4b4c-9934-544FC6D24D26}"">
            <Group clsid=""{6D4A79E4-529C-4481-ABD0-F5BD7EA93BA7}"" uid=""{8C4D8A58-4446-462A-93B5-8445AF7C0BBA}"" changed=""2019-10-30 00:07:18"" image=""2"" name=""Backup Operators"">
                <Properties groupName=""Backup Operators"" groupSid=""S-1-5-32-551"" removeAccounts=""0"" deleteAllGroups=""0"" deleteAllUsers=""0"" description="""" newName="""" action=""U"">
                    <Members>
                        <Member name=""TESTLAB\Domain Users"" action=""ADD"" sid=""S-1-5-21-3130019616-2776909439-2417379446-513""/>
                    </Members>
                </Properties>
        ...
        public async Task GPOLocalGroupProcessor_ReadGPOLocalGroups_Null_GPLink() {
            ...
            Assert.Empty(result.BackupOperators);            
        }
        ...
        [WindowsOnlyFact]
        public async Task GPOLocalGroupProcessor_ReadGPOLocalGroups_CollectsBackupOperators() {
            var mockLDAPUtils = new Mock<ILdapUtils>(MockBehavior.Loose);
            mockLDAPUtils.Setup(x => x.ResolveAccountName(It.IsAny<string>(), It.IsAny<string>()))
                .ReturnsAsync((true, new TypedPrincipal("S-1-5-21-3130019616-2776909439-2417379446-513", Label.User)));
            var gpcFileSysPath = Path.GetTempPath();
            ...
            var groupsXmlPath = Path.Join(gpcFileSysPath, "MACHINE", "Preferences", "Groups", "Groups.xml");
            Directory.CreateDirectory(Path.GetDirectoryName(groupsXmlPath));
            File.WriteAllText(groupsXmlPath, GroupXmlContentWithBackup);

            var gptTmplPath = Path.Join(gpcFileSysPath, "MACHINE", "Microsoft", "Windows NT", "SecEdit", "GptTmpl.inf");
            Directory.CreateDirectory(Path.GetDirectoryName(gptTmplPath));
            File.WriteAllText(gptTmplPath, GpttmplInfContentWithBackup);

            var mockComputerEntry = new Mock<IDirectoryObject>();
            var computerSid = "teapot";
            mockComputerEntry.Setup(x => x.TryGetSecurityIdentifier(out computerSid)).Returns(true);
            var mockComputerResults = new List<LdapResult<IDirectoryObject>>
                { LdapResult<IDirectoryObject>.Ok(mockComputerEntry.Object) };

            var mockGcpEntry = new Mock<IDirectoryObject>();
            string flags = "0";
            mockGcpEntry.Setup(x => x.TryGetProperty(LDAPProperties.GPCFileSYSPath, out gpcFileSysPath)).Returns(true);
            mockGcpEntry.Setup(x => x.TryGetProperty(LDAPProperties.Flags, out flags)).Returns(true);
            var mockGcpResults = new List<LdapResult<IDirectoryObject>>
                { LdapResult<IDirectoryObject>.Ok(mockGcpEntry.Object) };

            mockLDAPUtils.SetupSequence(x => x.Query(It.IsAny<LdapQueryParameters>(), It.IsAny<CancellationToken>()))
                .Returns(mockComputerResults.ToAsyncEnumerable)
                .Returns(mockGcpResults.ToAsyncEnumerable)
                .Returns(Array.Empty<LdapResult<IDirectoryObject>>().ToAsyncEnumerable);
            ...
            var processor = new GPOLocalGroupProcessor(mockLDAPUtils.Object);
            var testGPLinkProperty =
                "[LDAP:/o=foo/ou=foo Group (ABC123)/cn=foouser (blah)123/dc=somedomain;0;]";
            var result = await processor.ReadGPOLocalGroups(testGPLinkProperty, null);

            Assert.NotEmpty(result.BackupOperators);
            Assert.Contains(result.BackupOperators, principal =>
                principal.ObjectIdentifier == "S-1-5-21-3130019616-2776909439-2417379446-513" &&
                principal.ObjectType == Label.User);
        }
        ...
        [Fact]
        public async Task GPOLocalGroupProcessor_ProcessGPOXMLFile_BackupOperators() {
            var mockLDAPUtils = new Mock<ILdapUtils>();
            mockLDAPUtils.Setup(x => x.ResolveAccountName(It.IsAny<string>(), It.IsAny<string>()))
                .ReturnsAsync((true, new TypedPrincipal("S-1-5-21-3130019616-2776909439-2417379446-513", Label.User)));
            var gpcFileSysPath = Path.GetTempPath();
            ...
            Assert.Contains(actual, action =>
                action.TargetRid == GPOLocalGroupProcessor.LocalGroupRids.BackupOperators &&
                action.Target == GPOLocalGroupProcessor.GroupActionTarget.LocalGroup &&
                action.TargetSid == "S-1-5-21-3130019616-2776909439-2417379446-513");
        }
        ...
        [Fact]
        public async Task GPOLocalGroupProcess_ProcessGPOTemplateFile_BackupOperators() {
            var mockLDAPUtils = new Mock<ILdapUtils>();
            mockLDAPUtils.Setup(x => x.ResolveAccountName(It.IsAny<string>(), It.IsAny<string>()))
                .ReturnsAsync((true, new TypedPrincipal("S-1-5-21-3130019616-2776909439-2417379446-513", Label.User)));
            var gpcFileSysPath = Path.GetTempPath();
            ...
            Assert.Contains(actual, action =>
                action.TargetRid == GPOLocalGroupProcessor.LocalGroupRids.BackupOperators &&
                action.Target == GPOLocalGroupProcessor.GroupActionTarget.RestrictedMember &&
                action.TargetSid == "S-1-5-21-3130019616-2776909439-2417379446-513");
        }
